#!/usr/bin/env python3
"""
sync.py — BookStack Sync

Sincroniza os documentos técnicos do infra-lab (docs/architecture.md,
docs/adr.md, docs/runbook.md, README.md) para o BookStack do laboratório,
dentro da Prateleira "infra-lab".

Migrado de infra-lab-proxmox em 2026-09-22 (ver docs/adr.md ADR-014) e
adaptado para a estrutura real de docs deste repo — arquivo único por
tema, em vez de um diretório com um arquivo por página.
...
# Mapeamento de arquivo de origem (relativo à raiz do projeto) → nome do
# Livro no BookStack. O infra-lab usa um arquivo único por tema (não um
# diretório com uma página por arquivo, como o infra-lab-proxmox original).
DOC_FILE_MAPPING: dict[str, str] = {
...
SHELF_NAME = "infra-lab"
SHELF_DESCRIPTION = "Documentação do laboratório infra-lab"


def collect_doc_files(project_root: Path) -> list[tuple[Path, str]]:
    """
    Resolve DOC_FILE_MAPPING contra a raiz do projeto e retorna a lista de
    arquivos existentes, na ordem declarada em DOC_FILE_MAPPING.
    ...
    for rel_path, book_name in DOC_FILE_MAPPING.items():
        doc_file = project_root / rel_path
        if not doc_file.is_file():
            log_warn(f"Arquivo não encontrado: {doc_file} — pulando")
            continue

        files.append((doc_file, book_name))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Sincroniza README.md e docs/*.md (architecture, adr, runbook) "
            "para o BookStack do laboratório infra-lab."
        ),
