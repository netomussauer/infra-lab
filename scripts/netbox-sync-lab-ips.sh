...
#   ./netbox-sync-lab-ips.sh --netbox-url http://192.168.1.72
#
#   ./netbox-sync-lab-ips.sh -n -u http://192.168.1.72 192.168.1.0/24
#
#   ./netbox-sync-lab-ips.sh -v --timeout 3
#
# Código de saída:
...
# infra-lab usa uma única rede flat (sem VLANs) — ver docs/architecture.md §7.3
readonly -a DEFAULT_NETWORKS=(
  "192.168.1.0/24|Lab (rede única, sem VLANs)|yes"
)

# Tags padrão para todos os recursos criados
readonly NB_TAGS='[{"slug":"lab-k8s-01"},{"slug":"dev"},{"slug":"auto-discovered"}]'
...
Redes padrão (se não especificadas):
  192.168.1.0/24 Lab (rede única, sem VLANs)  (escaneada ativamente)
...
Exemplos:
  # Sincronizar a rede padrão do lab
  export NETBOX_TOKEN="token123"
  ./${SCRIPT_NAME} --netbox-url http://192.168.1.72

  # Modo dry-run
  ./${SCRIPT_NAME} -n -u http://192.168.1.72

  # Verbose + timeout maior por host
  ./${SCRIPT_NAME} -v --timeout 3
EOF
}
...
nb_get_ip() {
  local address="$1"   # ex: "192.168.1.100/32"
